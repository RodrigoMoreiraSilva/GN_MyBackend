...
ALTER TABLE [Empresas] DROP CONSTRAINT [FK_Empresas_GrupoEmpresa_GrupoEmpresaId];
GO
...
FROM [sys].[default_constraints] [d]
INNER JOIN [sys].[columns] [c] ON [d].[parent_column_id] = [c].[column_id] AND [d].[parent_object_id] = [c].[object_id]
WHERE ([d].[parent_object_id] = OBJECT_ID(N'[Empresas]') AND [c].[name] = N'GrupoEmpresaId');
IF @var1 IS NOT NULL EXEC(N'ALTER TABLE [Empresas] DROP CONSTRAINT [' + @var1 + '];');
ALTER TABLE [Empresas] ALTER COLUMN [GrupoEmpresaId] int NULL;
GO

ALTER TABLE [Empresas] ADD CONSTRAINT [FK_Empresas_GrupoEmpresa_GrupoEmpresaId] FOREIGN KEY ([GrupoEmpresaId]) REFERENCES [GrupoEmpresa] ([Id]) ON DELETE NO ACTION;
GO

INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
VALUES (N'20211105152810_Dominios_v2', N'5.0.11');
GO
